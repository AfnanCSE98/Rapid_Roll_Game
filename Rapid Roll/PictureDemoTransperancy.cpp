...
int x = 300, y = 200, r = 200,mode=0,condition=1,score_timer_counter=0;
...
char LIFE[10]={'L','I','F','E',language+48,'.','b','m','p','\0'};
/*
    function iDraw() is called again and again by the system.
*/
void showLife()
{
    iShowBMP(0,screenheit-50,LIFE);
    for(int i=1;i<=life;i++)
    {
        iShowBMP(150+(i-1)*50,screenheit-50,"LifeSymbol.bmp");
    }
}
...
//        if(mode) mover=mover+mover_speed;
    ...
    if(mode==0&&!(langmenu||shuru||highscor)){
        iClear();
        show_final_score();
    }
...
void iMouse(int button, int state, int mx, int my) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
            ...
            if(langmenu and (mx>=67&&mx<=440&&my>=400&&my<=531)) {
                ...
                LIFE[4]=SCORE[5]=Final_Score[11]=MainMenu[8]=highscore[9]=languages[8]=levels[5]=bhasha+48;
                fp=fopen("Game Info.txt","w");
